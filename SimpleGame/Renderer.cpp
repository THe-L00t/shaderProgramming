...
void Renderer::Initialize(int windowSizeX, int windowSizeY)
{
	...
	m_0Texture = CreatePngTexture("./texture/0.png", GL_NEAREST);
	m_1Texture = CreatePngTexture("./texture/1.png", GL_NEAREST);
	m_2Texture = CreatePngTexture("./texture/2.png", GL_NEAREST);
	m_3Texture = CreatePngTexture("./texture/3.png", GL_NEAREST);
	m_4Texture = CreatePngTexture("./texture/4.png", GL_NEAREST);
	m_5Texture = CreatePngTexture("./texture/5.png", GL_NEAREST);
	m_6Texture = CreatePngTexture("./texture/6.png", GL_NEAREST);
	m_7Texture = CreatePngTexture("./texture/7.png", GL_NEAREST);
...
void Renderer::CompileAllShaderPrograms()
{
	...
	m_FS = CompileShaders("./Shaders/FShader.vs", "./Shaders/FShader.fs");
...
void Renderer::CreateVertexBufferObjects()
{
		...
		=
	{
		...
		(1.f - temp)* size, (1.f - temp)* size, 0.f, 0.5f,	//Triangle1
		(0.f - temp)* size, (0.f - temp)* size, 0.f, 0.5f,
		(1.f - temp)* size, (1.f - temp)* size, 0.f, 0.5f,
		(0.f - temp)* size, (1.f - temp)* size, 0.f, 0.5f,	//Triangle2 / Quad1

		(0.f - temp)* size, (0.f - temp)* size, 0.f, 1.f,
		(1.f - temp)* size, (0.f - temp)* size, 0.f, 1.f,
		(1.f - temp)* size, (1.f - temp)* size, 0.f, 1.f,	//Triangle1
		(0.f - temp)* size, (0.f - temp)* size, 0.f, 1.f,
		(1.f - temp)* size, (1.f - temp)* size, 0.f, 1.f,
		(0.f - temp)* size, (1.f - temp)* size, 0.f, 1.f,	//Triangle2 / Quad2
	};
...
{
	GLuint texturId;
	GLuint textureId; glGenTextures(1, &m_RT0);
...
void Renderer::GeneralParticles(int numParticle)
{
	...
	int totalVerticsCount = numParticle * verticesCountPerParticle;
	int totalFloatCount = floatCountPerVertex * totalVerticsCount;
	...
	for (size_t i = 0; i < numParticle; i++)
	{
		...
		g = ((float)rand() / (float)RAND_MAX) * 2.f - 1.f;
		b = ((float)rand() / (float)RAND_MAX) * 2.f - 1.f;
...
void Renderer::DrawParticle()
{
	...
	int attribTime = glGetAttribLocation(shader, "a_sTime");
	...
	glVertexAttribPointer(attribValue, 1, GL_FLOAT, GL_FALSE, sizeof(float) * stride, (GLvoid*)(sizeof(float) * 3));


	glEnableVertexAttribArray(attribColor);
	glVertexAttribPointer(attribColor, 4, GL_FLOAT, GL_FALSE, sizeof(float) * stride, (GLvoid*)(sizeof(float) * 4));

	glEnableVertexAttribArray(attribTime);
	glVertexAttribPointer(attribTime, 1, GL_FLOAT, GL_FALSE, sizeof(float) * stride, (GLvoid*)(sizeof(float) * 8));
